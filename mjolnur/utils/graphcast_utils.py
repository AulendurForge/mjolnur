...
class GraphCastAeroTrainer:
    def __init__(
        ...
        self.model_config, self.task_config, self.params, self.state = (
            self.load_graphcast(graphcast_checkpoint_path)
        )
    ...
    def load_graphcast(self, checkpoint_name):
        ...
        print(
            f"  Model config: mesh_size={ckpt.model_config.mesh_size}, "
            f"resolution={ckpt.model_config.resolution}, "
            f"gnn_msg_steps={ckpt.model_config.gnn_msg_steps}"
        )
        print(
            f"  Task: {len(ckpt.task_config.pressure_levels)} pressure levels, "
    ...
    def initialize_model_functions(
        self, sample_inputs, sample_targets, sample_forcings
    ):
        ...
        if self.params is None:
            ...
            self.params, self.state = run_forward.init(
                jax.random.PRNGKey(0), sample_inputs, sample_targets, sample_forcings
            )
    ...
    def prepare_era5_for_graphcast(
        ...
        sixh_times_pd = pd.date_range(
            start=t0 - pd.Timedelta(hours=6), end=hourly_stop, freq="6h"
        )
        ...
        # -----------------------------
        inst = ds[[*got_surface_inst, *got_level]].sel(
            time=ds.time.dt.hour.isin([0, 6, 12, 18])
        )
        if got_level:
        ...
        if tp_name is None:
            raise ValueError(
                "Hourly total precipitation not found in ARCO dataset "
        ...
        def coarsen_to_1deg(dset: xr.Dataset) -> xr.Dataset:
            ...
            if not all(
                k in getattr(dset, "sizes", {}) for k in ("latitude", "longitude")
            ):
                return dset
            factor = int(1.0 / 0.25)
        ...
        if (
            "geopotential" in stat_1deg.data_vars
            and "geopotential_at_surface" not in stat_1deg
        ):
        ...
        print(
            f"inst_1deg times: {inst_1deg.time.values[:3]}... shape={len(inst_1deg.time)}"
        )
        print(
            f"tp6_1deg times: {tp6_1deg.time.values[:3]}... shape={len(tp6_1deg.time)}"
        )
        print(f"Expected times: {sixh_times[:3]}... shape={len(sixh_times)}")

        # Now the times truly match 1:1; require exact match at merge
        data_6h = xr.merge(
            [inst_1deg, tp6_1deg, stat_1deg], compat="override", join="inner"
        )
        ...
        required = set(self.task_config.input_variables) | set(
            self.task_config.target_variables
        )
        keep = [
            v
            for v in data_6h.data_vars
            if (v in required) or (v == "total_precipitation_6hr")
        ]
        ...
        for name in ["geopotential_at_surface", "land_sea_mask"]:
            if name in inputs and "time" in inputs[name].dims:
                raise RuntimeError(
                    f"Static input {name} has a time dim; expected none."
                )

        print(
            "  Inputs:",
            {k: tuple(v.sizes.values()) for k, v in inputs.data_vars.items()},
        )
        print(
            "  Targets:",
            {k: tuple(v.sizes.values()) for k, v in targets.data_vars.items()},
        )
        print(
            "  Forcings:",
            None
            if forcings is None
            else {k: tuple(v.sizes.values()) for k, v in forcings.data_vars.items()},
        )
    ...
    def validate_graphcast_predictions(
        self, predictions: xr.Dataset, targets: xr.Dataset
    ) -> Dict:
...
try:
    ...
    print(
        f"  Time steps: {len(time_range)} ({len(time_range) / 4:.0f} days of 6-hourly data)"
    )
